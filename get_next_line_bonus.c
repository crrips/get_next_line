...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/26 13:22:06 by apiloian          #+#    #+#             */
/*   Updated: 2023/02/26 13:22:06 by apiloian         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	while (1)
	{
		if (fill(ret, buf, &str[fd], fd) == NULL)
			return (NULL);
		if (!*buf)
			return (ret);
		ret = check(ret, buf, &i);
		while (ret[i])
		{
			...
			{
				free(str[fd]);
				str[fd] = ft_substr(ret, i + 1, ft_strlen(ret), 0);
